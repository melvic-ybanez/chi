...
package object eval {
  ...
  def generateAndShow(source: String)(implicit preferences: Preferences, env: Env): String =
    if (Preferences.showOutputInfo) generateAndShowWithInfo(source)
    else generateAndShowCode(source)
